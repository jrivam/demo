...
{
    public class PostgreSqlBuilderTable<T> : AbstractSqlBuilderTable<T> 
        where T : IEntity
    {
        public PostgreSqlBuilderTable(ISqlSyntaxSign syntaxsign)
            : base(syntaxsign)
        {
        ...
        public override (string commandtext, IList<SqlParameter> parameters) Insert(IEntityRepositoryProperties<T> entitytable)
        {
            ...
            {
                output += $"{(string.IsNullOrWhiteSpace(output) ? "SELECT " : ", ")}lastval()";
            }
