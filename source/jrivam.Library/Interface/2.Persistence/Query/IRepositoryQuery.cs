...
namespace jrivam.Library.Interface.Persistence.Query
{
    ...
    {
        (Result result, U data) SelectSingle (IQueryData<T, U> query, int maxdepth = 1);
        (Result result, IEnumerable<U> datas) Select (IQueryData<T, U> query, int maxdepth = 1, int top = 0);
